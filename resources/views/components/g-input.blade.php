...
@if ($type === 'textarea')
    <div class="relative {{ $size ?? 'w-full' }}">
        <textarea id="{{ $id ?? $name }}" name="{{ $name }}"
            ...
            placeholder=" " wire:model="{{ $wireModel ?? $name }}" {{ $attributes }} rows="{{ $rows }}"></textarea>
        <label for="{{ $id ?? $name }}"
...
@elseif($type === 'searchable-select')
    ...
    <div x-data="{
    ...
    }" class="relative {{ $size }}" @click.away="open = false">
        ...
        <button @click="open = !open" type="button"
            ...
            <button x-show="selected" x-cloak @click.stop="clearSelection()" type="button"
        ...
        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <div class="max-h-48 overflow-y-auto">
                                ...
                                <div class="ml-2 flex-shrink-0">
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium"
                                        :class="{
                                            ...
                                            'bg-gray-100 text-gray-800': !['active', 'inactive', 'suspended'].includes(
                                                option.status)
                                        }"
                ...
                <!-- No Results -->
                <div x-show="filteredOptions.length === 0" class="px-4 py-6 text-sm text-gray-500 text-center">
...
@elseif($type === 'date')
    <div class="relative {{ $size ?? 'w-full' }}">
        <input type="date" id="{{ $id ?? $name }}" name="{{ $name }}"
            class="block w-full px-3 py-2 md:py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-slate-800 focus:border-slate-800 sm:text-sm  peer {{ $class }}"
            wire:model="{{ $wireModel ?? $name }}" {{ $attributes }} />
        <label for="{{ $id ?? $name }}"
            class="absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-slate-800 peer-focus:dark:text-indigo-400 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 left-3">
            {{ $label }}
        </label>
...
@elseif($type === 'password')
    <div class="relative {{ $size ?? 'w-full' }}" x-data="{ showPassword: false }">
        <input :type="showPassword ? 'text' : 'password'" id="{{ $id ?? $name }}" name="{{ $name }}"
            ...
            placeholder=" " wire:model="{{ $wireModel ?? $name }}" {{ $attributes }} />
        <label for="{{ $id ?? $name }}"
            class="absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-slate-800 peer-focus:dark:text-indigo-400 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 left-3">
            {{ $label }}
        </label>
...
@else
    <div class="relative {{ $size ?? 'w-full' }}">
        <input type="{{ $type }}" id="{{ $id ?? $name }}" name="{{ $name }}"
            class="block w-full px-3 py-2 md:py-3 border border-gray-300 rounded-md  sm:text-sm  peer {{ $class }}"
            placeholder=" " wire:model="{{ $wireModel ?? $name }}" {{ $attributes }} />
        <label for="{{ $id ?? $name }}"
            class="absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-slate-800 peer-focus:dark:text-indigo-400 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 left-3">
            {{ $label }}
